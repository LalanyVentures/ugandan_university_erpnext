...
import { CheckSquare2, ChevronDown, ChevronRight, CreditCard, FileText, Printer, Receipt, RefreshCw, Search, Square, X } from 'lucide-react'
...
export function PaymentReceiptsPage() {
  ...
  function load() {
    ...
    Promise.all([registerMethod].map(() => callMethod<PaymentRow[]>(registerMethod)))
      .then(([rows]) => { setPayments(rows); setOpenSemesters(new Set(rows.flatMap(row => row.semesters?.length ? row.semesters : ['Unallocated / advance']))) })
  ...
  const semesterGroups = useMemo(() => {
    ...
    filtered.forEach(row => (row.semesters?.length ? row.semesters : ['Unallocated / advance']).forEach(semester => groups.set(semester, [...(groups.get(semester) ?? []), row])))
  ...
  function toggleSemester(semester:string, rows:PaymentRow[]) {
    const allSelected = rows.every(row => selected.has(String(row.name)))
    setSelected(current => { const next=new Set(current); rows.forEach(row => allSelected ? next.delete(String(row.name)) : next.add(String(row.name))); return next })
  }
  ...
  return <section className="records-page payment-receipts-page">
    ...
    <article className="card receipt-selection-card no-print">
      ...
      <div className="payment-semester-list">{loading?<div className="data-table-empty">Loading received payments…</div>:semesterGroups.length?semesterGroups.map(([semester,rows])=>{const semesterSelected=rows.every(row=>selected.has(String(row.name))), open=openSemesters.has(semester), semesterTotal=rows.reduce((sum,row)=>sum+Number(row.received_amount??row.paid_amount??0),0);return <article className="payment-semester-accordion" key={semester}><div className="payment-semester-heading"><button className="accordion-toggle" onClick={()=>setOpenSemesters(current=>{const next=new Set(current);next.has(semester)?next.delete(semester):next.add(semester);return next})}>{open?<ChevronDown size={17}/>:<ChevronRight size={17}/>}<span><strong>{semester}</strong><small>{rows.length} payment transaction{rows.length===1?'':'s'} · {ugx(semesterTotal)}</small></span></button><button className="semester-select" onClick={()=>toggleSemester(semester,rows)}>{semesterSelected?<CheckSquare2 size={17}/>:<Square size={17}/>} {semesterSelected?'Clear semester':'Select semester'}</button></div>{open?<div className="table-wrap"><table className="data-table receipt-picker-table"><thead><tr><th></th><th>Payment</th><th>Date</th><th>Student / Payer</th><th>Method</th><th>Reference</th><th>Amount</th><th>Action</th></tr></thead><tbody>{rows.map(row=>{const name=String(row.name),student=String(row.student??'');return <tr key={name} className={selected.has(name)?'selected-payment-row':''} onClick={()=>toggle(name)}><td><button className="table-check" aria-label={`Select ${name}`}>{selected.has(name)?<CheckSquare2 size={17}/>:<Square size={17}/>}</button></td><td><strong>{name}</strong></td><td>{formatDate(String(row.posting_date??''))}</td><td><strong>{String(row.student_name??row.party_name??row.party??'Student account')}</strong><small>{String(row.student_number??row.party??'')}</small></td><td>{String(row.mode_of_payment??'—')}</td><td>{String(row.reference_no??'—')}</td><td><strong>{ugx(row.received_amount??row.paid_amount)}</strong></td><td><div className="table-row-actions"><button onClick={event=>{event.stopPropagation();void generateEntries([name])}}><Receipt size={14}/>Receipt</button>{student?<button onClick={event=>{event.stopPropagation();setSearchParams({q:String(row.student_number??student)},{replace:true})}}><CreditCard size={14}/>Student payments</button>:null}</div></td></tr>})}</tbody></table></div>:null}</article>}):<div className="data-table-empty">No submitted incoming payments match this search.</div>}</div>
    </article>
