...
import { fetchListPage, type FrappeRow, type ListFilter, type ListPageQuery } from '../../../../api/frappe'
...
export function NativeRecordsPage({ view }: { view: RecordView }) {
  ...
  async function loadPage(query: ListPageQuery, signal?: AbortSignal) { const fixed:ListFilter[] = view === 'readiness' ? [{field:'clearance_type',operator:'=',value:'Graduation'}] : statusFilter(view,params.get('subtab')); return fetchListPage(item.doctype,item.fields,{...query,filters:[...fixed,...(query.filters??[])]},signal) }
